...
type
  ...
  HPDF_Error_Handler* = proc (error_no: HPDF_STATUS; detail_no: HPDF_STATUS;
                           user_data: pointer)
  HPDF_Alloc_Func* = proc (size: HPDF_UINT): pointer
  HPDF_Free_Func* = proc (aptr: pointer)
...
type
  HPDF_LineCap* = enum
    HPDF_BUTT_END = 0, HPDF_ROUND_END, HPDF_PROJECTING_SCUARE_END, HPDF_LINECAP_EOF
...
type
  ...
  HPDF_LineAnnotEndingStyle* = enum
  ...
  HPDF_LineAnnotCapPosition* = enum
    HPDF_LINE_ANNOT_CAP_INLINE = 0, HPDF_LINE_ANNOT_CAP_TOP
  HPDF_StampAnnotName* = enum
